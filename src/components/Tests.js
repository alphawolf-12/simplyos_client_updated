...
export default class Tests extends Component {
  ...
  componentDidMount() {
    fetch(`https://simplyopensource.in:5000/tests/category/${this.state.category_id}/4`)
    .then(res => res.json())
    .then(data => {
      if(data.length === 0) {
        ...
        fetch(`https://simplyopensource.in:5000/tests/category/${this.state.category_id}/8`)
        .then(res => res.json())
        .then(data => {
          if(data.length === 0) {
            document.getElementById('info').classList = "alert";
          ...
          fetch(`https://simplyopensource.in:5000/tests/category/${this.state.category_id}`)
          .then(res => res.json())
          .then(data => {
            if(data.length === 0) {
