  ...
  render() {
    const output = this.state.testsTitles.map((test, i) => (
      <Link to={`/test/${this.state.testsIds[i]}/${test}`}>
          <div className="test">
            <h2 style={{fontFamily: 'Lucida Sans'}}>{test}</h2>
          </div>
      </Link>
    )) 
    return (
      <div>
        ...
        <div className="user_profile" style={{marginTop: 20}}>
          ...
          <div className="user_profile_3" style={{display: "none"}}>
            <center><h2  style={{color: 'black', fontFamily: "Lucida Sans"}}>It looks like something is missing! Plese fill all the fields!</h2></center>
            <hr />
